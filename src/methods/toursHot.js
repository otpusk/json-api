...
export async function getToursHotBlock(token, blockId) {
    const { block, tours } = await makeCall({ endpoint: ENDPOINTS.hotBlock, query: { blockId, ...token }});

    return { block, tours };
}

export async function getToursHotTour(token, blockId, tourId) {
    const { searchedTour: { offers } = {}} = await makeCall({ endpoint: ENDPOINTS.hotTour, query: { blockId, id: tourId, ...token }});

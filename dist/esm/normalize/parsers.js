...
export const parseFlights = input => {
  ...
  return call(pipe(toPairs, map(_ref => {
    let [type, flights] = _ref;
    return [type, map(pipe(over(lensProp('seats'), seats => ({
      label: parseSeats(seats),
      value: seats
    })), over(lensProp('portFrDetails'), ifElse(Boolean, parsePortDetails, always(null))), over(lensProp('portToDetails'), ifElse(Boolean, parsePortDetails, always(null))), over(lensProp('luggage'), ifElse(isNil, always({}), identity))), flights)];
  }), fromPairs), {
    outbound,
    inbound
  });
...
export const parseChildrenAges = function () {
...
export const parsePeople = function (people) {
...
export const parseSubOperator = subOperator => call(when(data => !data || isEmpty(data), () => ({
  code: null,
  name: null
})), subOperator);
export const extractBookingData = applySpec({
  endDate: prop('date'),
  allow: pipe(prop('possible'), Boolean)
});
export const extractExternalOperatorData = ifElse(Boolean, applySpec({
  subOperatorName: propOr('', 'subOperator'),
  subHotelID: prop('sourceHotelId')
}), always({}));
export const scheduleOfBookingPaymentsMapper = map(applySpec({

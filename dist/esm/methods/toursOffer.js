...
const addCurrency = currencyLocal => when(always(currencyLocal), mergeLeft({
  currencyLocal
}));
const addLang = lang => when(always(lang), mergeLeft({
  lang
}));
const addShortCode = withShortCode => when(always(withShortCode), assoc('getShortOfferId', true));
export async function getToursOffer(token, offerId, fresh, currency, lang) {
  ...
  const {
    ...
    query: call(pipe(mergeLeft(token), addCurrency(currency), addLang(lang), addShortCode(withShortCode)), {
      offerId
    }),
  ...
  return result ? mergeAll([offers[result], {

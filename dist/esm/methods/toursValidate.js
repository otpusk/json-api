...
export async function getToursValidate(token, offerId) {
  ...
  if (token && token.city) {
    const {
      name = ''
    } = getDepartureCityById(token.city);
    token.city = name;
  }
  const {
    status,
  ...
  const newYears = Map(info && info.services || {}).filter(_ref => {
    let {
      type
    } = _ref;
    return type === NEW_YEAR_PAY;
  }).map(item => Map(item).update('price', normalizePrice).update('price', price => price * converter[currency]).update('price', uah => Map({
    usd: null,
    eur: null,
    uah: null

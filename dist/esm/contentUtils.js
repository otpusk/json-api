...
const convertText2Links = pipe(split(/(https?:\/\/[^\s]+|www\.[^\s]+)/g), map(when(getIsUrl, url => {
...
})), filter(text => text && text.trim()), arr => arr.join(''));
export const prepareContent2Render = pipe(clearHtmlTags, convertText2Links);